...
import { Heart, Star, Zap, type LucideIcon } from 'lucide-react';
import React, {
  useEffect,
  useLayoutEffect,
  useRef,
...
const SHAPES: Record<PeekRatingShape, LucideIcon> = { star: Star, heart: Heart, bolt: Zap };
...
const PeekRating: React.FC<PeekRatingProps> = ({
  ...
  idleColor = 'var(--color-ink-16)',
  tipColor = 'var(--color-sheet)',
  tipTextColor = 'var(--color-ink-100)',
  size = 32,
  lift = 6,
  magnify = 1.15,
  riseDuration = 320,
  ...
  const StarTag = (readOnly ? 'span' : 'button') as 'button';
  ...
  return (
    <div
      ...
      aria-label={readOnly ? `${value} de ${count}` : ariaLabel}
    ...
    >
      <div
        ref={rowRef}
        className="relative inline-flex touch-pan-y items-end pt-[var(--pr-room)] select-none [-webkit-tap-highlight-color:transparent] [-webkit-touch-callout:none]"
      ...
      >
        {interactive && showTip ? (
          <span
            ref={tipEl}
            className="pointer-events-none absolute top-0 left-0 inline-flex h-[calc(var(--pr-size)*0.6)] items-center rounded-full px-[calc(var(--pr-size)*0.3)] text-[length:calc(var(--pr-size)*0.38)] leading-none font-semibold tracking-[0.01em] whitespace-nowrap opacity-0 shadow-bevel [background:var(--pr-tip)] [color:var(--pr-tip-text)] [transition:transform_var(--pr-rise)_var(--pr-ease-out),opacity_180ms_ease] group-data-[instant=true]:[transition-duration:0ms] data-[show=true]:opacity-100 motion-reduce:[transition:opacity_180ms_ease]"
            aria-hidden="true"
          />
        ) : null}
        {Array.from({ length: count }, (_, i) => {
          const checked = value === i + 1;
          const label = labels[i] ? `${i + 1} de ${count}, ${labels[i]}` : `${i + 1} de ${count}`;
          return (
            <StarTag
              key={i}
              ...
              className="m-0 grid h-[calc(var(--pr-size)_+_8px)] w-[calc(var(--pr-size)_+_var(--pr-gap))] touch-manipulation place-items-center border-0 bg-transparent p-0 font-[inherit] text-inherit outline-none focus-visible:rounded-md focus-visible:outline-offset-2 focus-visible:[outline:2px_solid_color-mix(in_srgb,var(--pr-active)_70%,transparent)] [@media(hover:hover)_and_(pointer:fine)]:cursor-pointer [@media(pointer:coarse)]:min-h-11 [@media(pointer:coarse)]:min-w-11"
            ...
            >
              ...
              >
                <span
                  ...
                  className="block h-[var(--pr-size)] w-[var(--pr-size)] text-[var(--pr-idle)] [transition:color_160ms_ease] group-data-[instant=true]:[transition-duration:0ms] data-[lit=true]:text-[var(--pr-active)] [&>svg]:block [&>svg]:h-full [&>svg]:w-full"
                >
                  {icon ?? <ShapeIcon size={size} fill="currentColor" strokeWidth={1.5} />}
                </span>
              </span>
            </StarTag>
          );
